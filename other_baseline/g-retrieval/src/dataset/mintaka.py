...
print(f"add path {g_project_path}")
...
path = "/mnt/data/wangshu/hcarag/mintaka/KG"
...
def preprocess(entity_df, relation_df, relation_embedding, dataset, num_workers=16):
...
def read_graph():
    ...
    relation_df = pd.read_csv(f"{path}/relation_df.csv")
    r_embedding = pd.read_csv(f"{path}/relationships_embedding.csv")

    entity_df = entity_df[
        ["human_readable_id", "name", "description", "description_embedding"]
    ]
    relation_df = relation_df[
    ...
    r_embedding["description_embedding"] = r_embedding["description_embedding"].apply(
        lambda x: np.fromstring(x.strip("[]"), sep=",") if isinstance(x, str) else x
    )
    ...
    return entity_df, relation_df, r_embedding, dataset
