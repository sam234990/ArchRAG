...
def qa_read(query: str, passages: list, few_shot: list, client):
    ...
    instruction = (
        cot_system_instruction if len(passages) else cot_system_instruction_no_doc
    )
...
def parallel_qa_read(
    ...
    def process_sample(sample):
        ...
        if "retrieved" in sample:
            ...
            retrieved = [corpus[doc_id] for doc_id in sample["retrieved_id"]][
                : args.num_doc
            ]
        else:
            retrieved = []
        assert (
            len(retrieved) == args.num_doc
        ), f"sample {sample_id}: #retrieved {len(retrieved)} != args.num_doc {args.num_doc}"
        if len(retrieved):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        help="retrieval results or QA reading results",
        choices=["hotpotqa", "musique", "2wikimultihopqa"],
        required=True,
    )
    parser.add_argument(
        "--data", type=str, help="retrieval results or QA reading results"
    )
    parser.add_argument(
        "--retriever",
        type=str,
    ...
    if args.num_doc == 0:
        if args.dataset == "hotpotqa":
            ...
            prompt_path = (
                "data/ircot_prompts/2wikimultihopqa/no_context_cot_qa_codex.txt"
            )
        ...
        if os.path.isfile(output_path):  # resume from previous results
            ...
            for key in total_metrics.keys():
                total_metrics[key] = sum(
                    [sample[key] for sample in data if key in sample]
                )
        if args.dataset == "hotpotqa":
    ...
    parallel_qa_read(
        data, demos, args, client, output_path, total_metrics, processed_id_set
    )
    with open(output_path, "w") as f:
        json.dump(data, f)
    ...
    metric_str = " ".join(
        [f"{key}: {total_metrics[key] / len(data):.4f}" for key in total_metrics.keys()]
    )
    print(metric_str)
