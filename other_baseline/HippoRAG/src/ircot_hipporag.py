...
if __name__ == "__main__":
    ...
    colbert_configs = {
        "root": f"data/lm_vectors/colbert/{args.dataset}",
    ...
    rag = HippoRAG(
        ...
        node_specificity=not (args.wo_node_spec),
        sim_threshold=args.sim_threshold,
        colbert_config=colbert_configs,
        dpr_only=dpr_only,
    ...
    doc_ensemble_str = (
        f"doc_ensemble_{args.recognition_threshold}" if doc_ensemble else "no_ensemble"
    )
    ...
    for sample_idx, sample in tqdm(
        enumerate(data), total=len(data), desc="IRCoT retrieval"
    ):  # for each sample
        if args.dataset in ["hotpotqa", "2wikimultihopqa", "hotpotqa_train"]:
        ...
        retrieved_passages, scores, logs = retrieve_step(
            query, corpus, args.top_k, rag, args.dataset
        )
        ...
        while it < max_steps:  # for each iteration of IRCoT
            ...
            new_retrieved_passages, new_scores, logs = retrieve_step(
        ...
        for k in k_list:
            recall[k] = round(
                sum(1 for t in gold_items if t in retrieved_items[:k])
                / len(gold_items),
                4,
            )
