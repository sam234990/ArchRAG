...
def run_colbertv2_index(
    ...
    index_name: str,
    corpus_tsv_path: str,
    checkpoint_path="/mnt/data/wangshu/llm_lm/exp/colbertv2.0",
    overwrite=False,
):
    with Run().context(
        RunConfig(nranks=1, experiment="colbert", root=f"exp/{dataset_name}/")
    ):
...
if __name__ == "__main__":
    ...
    if args.corpus == "hotpotqa_1000_proposition":
        corpus = json.load(
            open("data/dense_x_retrieval/hotpotqa_proposition_corpus.json", "r")
        )
        corpus_contents = [
            item["title"] + "\t" + item["propositions"].replace("\n", " ")
            for item in corpus
        ]
        ...
        corpus = json.load(
            open("data/dense_x_retrieval/musique_proposition_corpus.json", "r")
        )
        corpus_contents = [
            item["title"] + "\t" + item["propositions"].replace("\n", " ")
            for item in corpus
        ...
        corpus = json.load(
            open("data/dense_x_retrieval/2wikimultihopqa_proposition_corpus.json", "r")
        )
        corpus_contents = [
            item["title"] + "\t" + item["propositions"].replace("\n", " ")
            for item in corpus
    ...
    run_colbertv2_index(
        args.dataset,
        args.corpus + "_nbits_2",
        corpus_tsv_path,
        "/mnt/data/wangshu/llm_lm/exp/colbertv2.0",
        overwrite=True,
    )
