        ...
        with torch.no_grad():
            query_embedding = (
                mean_pooling_embedding_with_normalization(
                    query, self.tokenizer, self.model, self.device
                )
                .detach()
                .cpu()
                .numpy()
...
class SentenceTransformersRetriever(DocumentRetriever):
    def __init__(
        self, model_name: str, faiss_index: str, corpus, device="cuda", norm=True
    ):
        """
...
def retrieve_step(
    query: str, corpus, top_k: int, retriever: DocumentRetriever, dataset: str
):
...
def reason_step(
    dataset, few_shot: list, query: str, passages: list, thoughts: list, client
):
    """
    ...
    for sample in few_shot:
        ...
        if (
            num_tokens_by_tiktoken(
                ircot_reason_instruction + prompt_demo + cur_sample + prompt_user
            )
            < 15000
        ):
    ...
    messages = ChatPromptTemplate.from_messages(
        [
            SystemMessage(ircot_reason_instruction + "\n\n" + prompt_demo),
...
def process_sample(idx, sample, args, corpus, retriever, client, processed_ids):
    ...
    retrieved_passages, scores = retrieve_step(
        query, corpus, args.top_k, retriever, args.dataset
    )
    ...
    for it in range(1, max_steps):
        ...
        if "So the answer is:" in new_thought:
            break
        new_retrieved_passages, new_scores = retrieve_step(
            new_thought, corpus, args.top_k, retriever, args.dataset
        )

        for passage, score in zip(new_retrieved_passages, new_scores):
            if passage in retrieved_passages_dict:
                retrieved_passages_dict[passage] = max(
        ...
        sorted_passages_scores = sorted(
            zip(retrieved_passages, scores), key=lambda x: x[1], reverse=True
        )
    ...
    for k in k_list:
        recall[k] = sum(1 for t in gold_items if t in retrieved_items[:k]) / len(
            gold_items
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--unit", type=str, choices=["hippo", "proposition"], default="hippo"
    )
    parser.add_argument(
        "--num_demo",
        type=int,
        default=1,
        help="the number of documents in the demonstration",
        required=True,
    )
    parser.add_argument("--max_steps", type=int)
    parser.add_argument(
        "--top_k", type=int, default=8, help="retrieving k documents at each step"
    )
    parser.add_argument(
        "--thread",
        type=int,
        default=6,
        help="number of threads for parallel processing, 1 for sequential processing",
    )
    ...
    colbert_configs = {
        "root": f"data/lm_vectors/colbertv2/{args.dataset}",
    ...
    if args.retriever == "bm25":
        if args.unit == "hippo":
            ...
            retriever = BM25Retriever(
                index_name=f"{args.dataset}_{len(corpus)}_proposition_bm25"
            )
        ...
        if args.dataset == "hotpotqa":
            if args.unit == "hippo":
                faiss_index = faiss.read_index(
                    "data/hotpotqa/hotpotqa_facebook_contriever_hippo_ip_norm.index"
                )
            elif args.unit == "proposition":
                faiss_index = faiss.read_index(
                    "data/hotpotqa/hotpotqa_proposition_ip_norm.index"
                )
        elif args.dataset == "musique":
            if args.unit == "hippo":
                faiss_index = faiss.read_index(
                    "data/musique/musique_facebook_contriever_hippo_ip_norm.index"
                )
            elif args.unit == "proposition":
                faiss_index = faiss.read_index(
                    "data/musique/musique_proposition_ip_norm.index"
                )
        elif args.dataset == "2wikimultihopqa":
            if args.unit == "hippo":
                faiss_index = faiss.read_index(
                    "data/2wikimultihopqa/2wikimultihopqa_facebook_contriever_hippo_ip_norm.index"
                )
            elif args.unit == "proposition":
                faiss_index = faiss.read_index(
                    "data/2wikimultihopqa/2wikimultihopqa_proposition_ip_norm.index"
                )
        ...
        if args.dataset == "hotpotqa":
            if args.unit == "hippo":
                faiss_index = faiss.read_index(
                    "data/hotpotqa/hotpotqa_sentence-transformers_gtr-t5-base_hippo_ip_norm.index"
                )
            elif args.unit == "proposition":
                faiss_index = faiss.read_index(
                    "data/hotpotqa/hotpotqa_sentence-transformers_gtr-t5-base_proposition_ip_norm.index"
                )
        elif args.dataset == "musique":
            if args.unit == "hippo":
                faiss_index = faiss.read_index(
                    "data/musique/musique_sentence-transformers_gtr-t5-base_hippo_ip_norm.index"
                )
            elif args.unit == "proposition":
                faiss_index = faiss.read_index(
                    "data/musique/musique_sentence-transformers_gtr-t5-base_proposition_ip_norm.index"
                )
        elif args.dataset == "2wikimultihopqa":
            if args.unit == "hippo":
                faiss_index = faiss.read_index(
                    "data/2wikimultihopqa/2wikimultihopqa_sentence-transformers_gtr-t5-base_hippo_ip_norm.index"
                )
            elif args.unit == "proposition":
                faiss_index = faiss.read_index(
                    "data/2wikimultihopqa/2wikimultihopqa_sentence-transformers_gtr-t5-base_proposition_ip_norm.index"
                )
    ...
    try:
        ...
        if args.dataset in ["hotpotqa", "2wikimultihopqa"]:
            processed_ids = {
                sample["_id"] for sample in results if "retrieved" in sample
            }
        elif args.dataset in ["musique"]:
            processed_ids = {
                sample["id"] for sample in results if "retrieved" in sample
            }
        else:
            raise NotImplementedError(f"Dataset {args.dataset} not implemented")
        for sample in results:
            if "recall" in sample:
                total_recall = {
                    k: total_recall[k] + sample["recall"][str(k)] for k in k_list
                }
    except Exception as e:
    ...
    if read_existing_data:
        print(
            f"All samples have been already in the result file ({output_path}), exit."
        )
        exit(0)
