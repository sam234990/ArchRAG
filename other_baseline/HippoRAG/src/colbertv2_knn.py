...
def retrieve_knn(kb, queries, duplicate=True, nns=100):
    ...
    with open("data/lm_vectors/colbert/corpus.tsv", "w") as f:  # save to tsv
        for pid, p in enumerate(kb):
            f.write(f'{pid}\t"{p}"' + "\n")
    print("Saved corpus to data/lm_vectors/colbert/corpus.tsv")

    with open("data/lm_vectors/colbert/queries.tsv", "w") as f:  # save to tsv
        for qid, q in enumerate(queries):
            f.write(f"{qid}\t{q}" + "\n")
    print("Saved queries to data/lm_vectors/colbert/queries.tsv")
...
if __name__ == "__main__":
    ...
    output_path = "data/lm_vectors/colbert/nearest_neighbor_{}.p".format(
        string_filename.split("/")[1].split(".")[0]
    )
