...
python src/openie_with_retrieval_option_parallel.py --dataset $data --llm $llm_api \
    --model_name $extraction_model --run_ner --num_passages all \
    --num_processes $num_processes # NER and OpenIE for passages
python src/named_entity_extraction_parallel.py --dataset $data --llm $llm_api \
    --model_name $extraction_model --num_processes $num_processes # NER for queries
