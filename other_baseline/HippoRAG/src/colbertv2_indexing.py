...
def colbertv2_index(
    ...
    corpus_tsv_file_path = (
        f"data/lm_vectors/colbert/{dataset_name}_{exp_name}_{len(corpus_processed)}.tsv"
    )
...
if __name__ == "__main__":
    ...
    colbertv2_index(
        corpus_contents, args.dataset, "corpus", checkpoint_path, overwrite=True
    )

    kb_phrase_dict = pickle.load(open(args.phrase, "rb"))
    phrases = np.array(list(kb_phrase_dict.keys()))[
        np.argsort(list(kb_phrase_dict.values()))
    ]
