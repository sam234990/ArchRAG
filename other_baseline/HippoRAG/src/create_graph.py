...
def create_graph(
    ...
    possible_files = glob(
        "output/openie_{}_results_{}_{}_*.json".format(
            dataset, extraction_type, extraction_model
        )
    )
    max_samples = np.max(
        [
            int(file.split("{}_".format(extraction_model))[1].split(".json")[0])
    ...
    extracted_file = json.load(
        open(
            "output/openie_{}_results_{}_{}_{}.json".format(
                dataset, extraction_type, extraction_model, max_samples
            ),
            "r",
        )
    )
    ...
    print(
        "Correct Wiki Format: {} out of {}".format(
            correct_wiki_format, len(extracted_triples)
        )
    )

    try:
        queries_full = pd.read_csv(
            "output/{}_queries.named_entity_output.tsv".format(dataset), sep="\t"
        )
    ...
    if create_graph_flag:
        ...
        json.dump(
            passage_json,
            open(
                "output/{}_{}_graph_passage_chatgpt_openIE.{}_{}.{}.subset.json".format(
                    dataset, graph_type, phrase_type, extraction_type, version
                ),
                "w",
            ),
        )
        json.dump(
            node_json,
            open(
                "output/{}_{}_graph_nodes_chatgpt_openIE.{}_{}.{}.subset.json".format(
                    dataset, graph_type, phrase_type, extraction_type, version
                ),
                "w",
            ),
        ...
        pickle.dump(
            graph,
            open(
                "output/{}_{}_graph_fact_doc_edges_{}_{}.{}.subset.p".format(
                    dataset, graph_type, phrase_type, extraction_type, version
                ),
                "wb",
            ),
        ...
        if cosine_sim_edges:
            if "colbert" in retriever_name:
                kb_similarity = pickle.load(
                    open(
                        "data/lm_vectors/colbert/nearest_neighbor_kb_to_kb.p".format(
                ...
                kb_similarity = pickle.load(
                    open(
                        "data/lm_vectors/{}_mean/nearest_neighbor_kb_to_kb.p".format(
                            processed_retriever_name
                        ),
                        "rb",
                    )
            ...
            pickle.dump(
                synonym_candidates,
                open(
                    "output/{}_similarity_edges_mean_{}_thresh_{}_{}_{}.{}.subset.p".format(
                        dataset,
                        threshold,
                        phrase_type,
                        extraction_type,
        ...
        if similarity_max == 1.0:
            ...
            pickle.dump(
                graph_plus,
                open(
                    "output/{}_{}_graph_mean_{}_thresh_{}_{}_sim_max_{}_{}.{}.subset.p".format(
                        dataset,
                        graph_type,
                        threshold,
                        phrase_type,
        ...
        json.dump(
            graph_json,
            open(
                "output/{}_{}_graph_chatgpt_openIE.{}_{}.{}.subset.json".format(
                    dataset, graph_type, phrase_type, extraction_type, version
                ),
                "w",
            ),
