...
COLBERT_CKPT_DIR = "/mnt/data/wangshu/llm_lm/exp/colbertv2.0"
...
class HippoRAG:
    ...
    def __init__(
        ...
        self.graph_creating_retriever_name_processed = (
            graph_creating_retriever_name.replace("/", "_").replace(".", "")
        )
        ...
        if colbert_config is None:
            self.colbert_config = {
                "root": f"data/lm_vectors/colbert/{corpus_name}",
                "doc_index_name": "nbits_2",
                "phrase_index_name": "nbits_2",
            }
        else:
            self.colbert_config = (
                colbert_config  # a dict, 'root', 'doc_index_name', 'phrase_index_name'
            )
        ...
        try:
            self.named_entity_cache = pd.read_csv(
                "output/{}_queries.named_entity_output.tsv".format(self.corpus_name),
                sep="\t",
            )
        ...
        if (doc_ensemble or dpr_only) and self.linking_retriever_name not in [
            "colbertv2",
            "bm25",
        ]:
    ...
    def get_extraction_by_passage_idx(self, passage_idx, chunk=False):
        ...
        for item in self.extracted_triples:
            if not chunk and item["idx"] == passage_idx:
                ...
                item["idx"] == passage_idx or item["idx"].startswith(passage_idx + "_")
            ):
                return item
        return None
    ...
    def load_index_files(self):
        index_file_pattern = "output/openie_{}_results_{}_{}_*.json".format(
            self.corpus_name, self.extraction_type, self.extraction_model_name_processed
        )
        ...
        extracted_file = json.load(
            open(
                "output/openie_{}_results_{}_{}_{}.json".format(
                    self.corpus_name,
        ...
        if self.corpus_name == "hotpotqa":
            ...
            self.dataset_df["paragraph"] = [
                s["passage"] for s in self.extracted_triples
            ]
        if self.corpus_name == "hotpotqa_train":
            ...
            self.dataset_df = pd.DataFrame(
                [p["passage"] for p in self.extracted_triples]
            )
            self.dataset_df["paragraph"] = [
                s["passage"] for s in self.extracted_triples
            ]
        ...
        self.kb_node_phrase_to_id = pickle.load(
            open(
                "output/{}_{}_graph_phrase_dict_{}_{}.{}.subset.p".format(
                    self.corpus_name,
                    self.graph_type,
                    self.phrase_type,
                    self.extraction_type,
        ...
        self.lose_fact_dict = pickle.load(
            open(
                "output/{}_{}_graph_fact_dict_{}_{}.{}.subset.p".format(
                    self.corpus_name,
                    self.graph_type,
                    self.phrase_type,
                    self.extraction_type,
        ...
        self.docs_to_facts = pickle.load(
            open(
                "output/{}_{}_graph_doc_to_facts_{}_{}.{}.subset.p".format(
                    self.corpus_name,
                    self.graph_type,
                    self.phrase_type,
                    self.extraction_type,
        ...
        self.facts_to_phrases = pickle.load(
            open(
                "output/{}_{}_graph_facts_to_phrases_{}_{}.{}.subset.p".format(
                    self.corpus_name,
                    self.graph_type,
                    self.phrase_type,
                    self.extraction_type,
        ...
        self.docs_to_facts_mat = pickle.load(
            open(
                "output/{}_{}_graph_doc_to_facts_csr_{}_{}.{}.subset.p".format(
                    self.corpus_name,
                    self.graph_type,
                    self.phrase_type,
                    self.extraction_type,
        ...
        self.facts_to_phrases_mat = pickle.load(
            open(
                "output/{}_{}_graph_facts_to_phrases_csr_{}_{}.{}.subset.p".format(
                    self.corpus_name,
                    self.graph_type,
                    self.phrase_type,
                    self.extraction_type,
        ...
        graph_file_path = (
            "output/{}_{}_graph_mean_{}_thresh_{}_{}_{}.{}.subset.p".format(
                self.corpus_name,
                self.graph_type,
        ...
        if os.path.isfile(graph_file_path):
            self.graph_plus = pickle.load(
                open(graph_file_path, "rb")
            )  # (phrase1 id, phrase2 id) -> the number of occurrences
        else:
    ...
    def load_node_vectors(self):
        ...
        if os.path.isfile(encoded_string_path):
            ...
            kb_node_phrase_embeddings_path = (
                "data/lm_vectors/{}_mean/{}_kb_node_phrase_embeddings.p".format(
                    self.linking_retriever_name_processed, self.corpus_name
                )
            )
            if os.path.isfile(kb_node_phrase_embeddings_path):
                self.kb_node_phrase_embeddings = pickle.load(
                    open(kb_node_phrase_embeddings_path, "rb")
                )
                if len(self.kb_node_phrase_embeddings.shape) == 3:
                    self.kb_node_phrase_embeddings = np.squeeze(
                        self.kb_node_phrase_embeddings, axis=1
                    )
                self.logger.info(
                    "Loaded phrase embeddings from: "
                    + kb_node_phrase_embeddings_path
                    + ", shape: "
                ...
                self.kb_node_phrase_embeddings = self.embed_model.encode_text(
                    self.phrases.tolist(), return_cpu=True, return_numpy=True, norm=True
                )
                pickle.dump(
    ...
    def get_dpr_doc_embedding(self):
        cache_filename = "data/lm_vectors/{}_mean/{}_doc_embeddings.p".format(
            self.linking_retriever_name_processed, self.corpus_name
        )
